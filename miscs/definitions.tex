...
\begin{definition} {Semimetric, Metric}
    Set \(X\). Let \(d: X \times X \to \bbr\) \st \begin{enumerate}
        \item \(d(x, y) = d(y, x) \geq 0\)
        \item $d(x, x) = 0$
        \item $d(x, z) \leq d(x, y) + d(y, z)$
    \end{enumerate}

    Then $d$ is a \textbf{semimetric}. A \textbf{metric} is a semimetric $d$ \st $d(x, y) > 0$ when $x \neq y$
\end{definition}
